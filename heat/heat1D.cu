...
void cputimer_stop(const char *info)
{
    ...
    printf("Timing - %-50s => \tElasped %.0f microseconds \n", info, time);
}
...
{
    // Stencil from the finite difference discretization of the equation
...
int main(int argc, char **argv)
{
    ...
    cusparseDnVecDescr_t Tdescriptor; // Vec descriptor needed by SpMV
    cusparseDnVecDescr_t Ydescriptor; // Immediate vec descriptor needed by SpMV
